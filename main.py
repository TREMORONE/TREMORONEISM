...
TOKEN = '1698991884:AAHyXRIk91ujYnO5OBjNdDBcWAG29i9JG7E'

THREADS_LIMIT = 1300

chat_ids_file = 'chat_ids.txt'

ADMIN_CHAT_ID =1213555559 

group_id = -1001442900987
...
def handle_message_received(message):
  ...
  if user_status == 'member' or user_status == 'administrator' or user_status == 'creator':
   ...
   if text == "Добавить партнер" and chat_id == ADMIN_CHAT_ID:
    ...
    bot.send_message(chat_id, 'Владелец бота: @nexochy \nПо вопросам сотрудничества обращаться в ЛС сюда @nexochy \nБот работает пока что только на Россию и Украину')

   elif text == '🔥💣БОМБЕР':
    bot.send_message(chat_id, 'Введите номер в формате:\n🇷🇺 79xxxxxxxxx\n🇺🇦 380xxxxxxxxx')
            
   elif text == '📈Статистика':
    bot.send_message(chat_id, 'Бот активно и без перебоев работает с 09.03.2021')

   elif text == '💰Поддержать':
    bot.send_message(chat_id, 'Кто может материально помочь на развитие бота\nВот реквизиты\nQIWI карта: ' + '<pre>4890 4947 2039 9971</pre>', parse_mode="HTML" )
 ...
 Купить: @nexochy  """)    
    ...
    bot.send_message(chat_id, 'Пока-что партнера нет')
